#include "product.h"
#include "book.h"
#include "util.h"
#include <set>
#include <string>
#include <vector>
#include <iostream>
#include <sstream>
#include <fstream>

using namespace std;
Book::Book(const string category, const string name, double price, int qty, string isbn, string author) : Product(category, name, price, qty), isbn_(isbn), author_(author) {}
Book::~Book() {}
std::set<std::string> Book::keywords() const {
    set<string> keywords;
    string temp = name_;
    string temp2 = author_;
    string temp3 = isbn_;
    string temp4 = category_;
    string temp5 = price_;
    string temp6 = qty_;
    string temp7 = temp + temp2 + temp3 + temp4 + temp5 + temp6;
    vector<string> temp8 = parseStringToWords(temp7);
    for (int i = 0; i < temp8.size(); i++) {
        keywords.insert(temp8[i]);
    }
    return keywords;
}
std::string Book::displayString() const {
    stringstream ss;
    ss << name_ << endl << "Author: " << author_ << " ISBN: " << isbn_ << endl << price_ << " " << qty_ << " left.";
    return ss.str();
}
void Book::dump(std::ostream& os) const {
    os << category_ << endl << name_ << endl << price_ << endl << qty_ << endl << isbn_ << endl << author_ << endl;
}